...
// For each machine, find its block boundaries then find each lang block
// Use a wider search window

function addScenarioToMachine(machineId, lang, threatId, scenario) {
  // Find machine block
  const machineMarker = `'${machineId}': {`;
  const machineIdx = c.indexOf(machineMarker);
  if (machineIdx === -1) return false;

  // Find the END of this machine block (next top-level machine)
  // Look for the next occurrence of a pattern like "  'some-id': {" after machineIdx
  let machineEndIdx = c.length;
  const nextMachineRe = /\n  '[a-z][^']+': \{/g;
  nextMachineRe.lastIndex = machineIdx + machineMarker.length;
  const match = nextMachineRe.exec(c);
  if (match) machineEndIdx = match.index;

  const machineBlock = c.slice(machineIdx, machineEndIdx);
  
  // Find lang block within machine block
  const langMarker = `\n    ${lang}: {`;
  const langRelIdx = machineBlock.indexOf(langMarker);
  if (langRelIdx === -1) return false;
  
  const langAbsIdx = machineIdx + langRelIdx;
  
  // Find threat within lang block
  const threatMarker = `        ${threatId}: {`;
  const threatRelIdx = machineBlock.indexOf(threatMarker, langRelIdx);
  if (threatRelIdx === -1) return false;
  
  const threatAbsIdx = machineIdx + threatRelIdx;
  
  // Find closing } of threat entry
  const closeIdx = c.indexOf('}', threatAbsIdx + threatMarker.length);
  
  const chunk = c.slice(threatAbsIdx, closeIdx + 1);
  if (chunk.includes('scenario:')) return true;
  
  const safeScenario = scenario.replace(/'/g, '\u2019');
  c = c.slice(0, closeIdx) + `, scenario: '${safeScenario}'` + c.slice(closeIdx);
  return true;
}
...
const data = {
  'lathe-conventional': {
    ...
    de: {
      ...
      lc04: 'Bearbeitung ohne Schutzbrille; Späne Richtung Gesicht',
      ...
      lc18: 'Getriebeservice mit offenem Gehäuse; Wellen drehen sich',
    ...
    fr: {
      ...
      lc02: "Changement d'outil manuel avec entraînement en marche; contact avec porte-outil",
      lc03: "Tournage longue barre sans lunette; vitesse > 300 tr/min",
      lc04: "Usinage sans lunettes; copeaux vers le visage",
    ...
    it: {
      ...
      lc02: "Cambio utensile manuale con azionamento in moto; contatto portautensile",
      lc03: "Tornitura barra lunga senza lunetta; velocità > 300 giri/min",
      lc04: "Lavorazione senza occhiali; trucioli verso il viso",
      ...
      lc07: "Primo avviamento dopo riparazione; senso rotazione non verificato",
      ...
      lc14: "Livellamento con cunei; macchina instabile prima ancoraggio",
      ...
      lc20: "Pulizia macchina dopo turno; rimozione manuale trucioli senza gancio",
      ...
      lc25: "Smontaggio tornio prima rottamazione; parti pesanti senza carroponte",
    },
    es: {
      lc01: "Operador retira pieza con husillo girando; sin protección mandril",
      lc02: "Cambio manual herramienta con accionamiento en marcha; contacto portaherramienta",
      lc03: "Torneado barra larga sin luneta; velocidad > 300 rpm",
      lc04: "Mecanizado sin gafas de seguridad; virutas hacia la cara",
      lc05: "Ajuste carro a baja velocidad; manos cerca zona mandril",
      lc06: "Conexión cables 400V sin corte principal",
      lc07: "Primera puesta en marcha tras reparación; sentido giro no verificado",
      ...
      lc11: "Trabajo con pieza desequilibrada en mandril 4 garras",
      lc12: "Acumulación virutas y vapores; chispa del mandril",
...
for (const [machineId, langs] of Object.entries(data)) {
  for (const [lang, threats] of Object.entries(langs)) {
    for (const [threatId, scenario] of Object.entries(threats)) {
      const result = addScenarioToMachine(machineId, lang, threatId, scenario);
      if (result) addedCount++;
      else { notFoundCount++; console.log(`NOT FOUND: ${machineId}/${lang}/${threatId}`); }
    }
  }
}

fs.writeFileSync('C:/Projects/riskpro/frontend/src/i18n/machinesI18n.ts', c, 'utf8');
console.log(`\nAdded: ${addedCount}, Not found: ${notFoundCount}`);
