...
for (const [lang, entries] of Object.entries(newEntries)) {
  // Re-read file each time to get fresh indices
  ...
  if (machineBlock.includes('lc08') && machineBlock.includes(`\n    ${lang}: {`)) {
    // Check if lc08 is in this lang block
    const langRelIdx = machineBlock.indexOf(`\n    ${lang}: {`);
    const lc08RelIdx = machineBlock.indexOf('lc08:', langRelIdx);
    if (lc08RelIdx > -1 && lc08RelIdx < langRelIdx + 5000) {
      console.log(`${lang}: lc08 already exists, skipping`);
      continue;
    }
  }
  ...
  const lc07RelIdx = machineBlock.indexOf('        lc07:', langRelIdx);
  if (lc07RelIdx === -1) { console.log(`lc07 not found in ${lang}`); continue; }
  
  const lc07End = machineBlock.indexOf('},', lc07RelIdx) + 2;
